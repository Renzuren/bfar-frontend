...
const AdminDashboard = () => {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      const payload = {
        ...
        middle_name: newUser.middleName.trim(),
  ...
  const filteredUsers = users.filter((u) => {
    ...
    return (
      ...
      getOrgName(u.org_id).toLowerCase().includes(q)
    );
  });
        ...
        <div className="flex w-full items-center justify-between px-3 py-3 sm:px-5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden items-center gap-3 rounded-full border border-slate-200/80 bg-white/90 py-1.5 pl-1.5 pr-3 shadow-sm backdrop-blur-sm sm:flex">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-purple-600 text-xs font-bold text-white shadow-md">
                {initials}
              </div>
      ...
      <main className="w-full px-3 pb-24 pt-0 sm:px-4">
        <div className="space-y-8">
          ...
          <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-violet-900 px-6 py-8 sm:px-10 sm:py-12 text-white shadow-2xl shadow-slate-900/20 text-left">
            ...
            <div className="relative">
              ...
              <p className="max-w-2xl text-base text-slate-300">
                Monitor all users and organizations across the platform from one central dashboard.
              </p>
            </div>
          </section>
          ...
          <section className="grid grid-cols-2 gap-5 lg:grid-cols-4">
            {[
              { label: 'Total Users', value: users.length, icon: Users, color: 'cyan', bg: 'bg-cyan-500', iconBg: 'bg-cyan-50', iconText: 'text-cyan-600' },
              { label: 'Organizations', value: organizations.length, icon: Building2, color: 'indigo', bg: 'bg-indigo-500', iconBg: 'bg-indigo-50', iconText: 'text-indigo-600' },
              { label: 'Total Projects', value: totalProjects, icon: FolderKanban, color: 'emerald', bg: 'bg-emerald-500', iconBg: 'bg-emerald-50', iconText: 'text-emerald-600' },
              { label: 'Active Users', value: users.filter((u) => u.status === 'active').length, icon: UserPlus, color: 'amber', bg: 'bg-amber-500', iconBg: 'bg-amber-50', iconText: 'text-amber-600' },
            ].map((stat) => {
              ...
              <div className="flex items-center gap-3">
                <div className="relative w-full sm:w-72">
                  ...
                  <input
                    ...
                    placeholder={activeTab === 'users' ? 'Search users...' : 'Search organizations...'}
                    className="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-2.5 pl-10 pr-4 text-sm text-slate-900 outline-none transition-all focus:border-violet-400 focus:bg-white focus:ring-4 focus:ring-violet-100"
                  />
                </div>
                <Button
                  onClick={fetchData}
                ...
                <Button
                  onClick={() => activeTab === 'users' ? setAddUserOpen(true) : setAddOrgOpen(true)}
                  className="bg-violet-600 text-white hover:bg-violet-700"
                >
                  <Plus className="mr-1.5 h-4 w-4" />
                  {activeTab === 'users' ? 'Add User' : 'Add Organization'}
                </Button>
              </div>
            </div>
          </section>
                ...
                <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-left">
                  ...
                  <p className="mx-auto mb-6 max-w-md text-sm text-slate-500">
                    {searchQuery
                      ...
                      : 'No users registered yet. Create the first user account.'}
                  </p>
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredUsers.map((u) => {
                        const userInitials = (u.full_name || u.email || 'U')
                          ...
                          .join('') || 'U';
                        return (
                          <tr key={u.id || u.uid} className="transition hover:bg-slate-50/80">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  {u.role === 'admin' && (
                                    <span className="inline-flex items-center rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-bold uppercase text-violet-600 ring-1 ring-violet-200">
                                      Admin
                                    </span>
                                  )}
                                </div>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={u.org_id ? 'text-slate-700 font-medium' : 'text-slate-400 italic'}>
                                  {getOrgName(u.org_id)}
                                </span>
                              </div>
                            </td>
            ...
            <>
              {filteredOrgs.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-left">
                  ...
                  <p className="mx-auto mb-6 max-w-md text-sm text-slate-500">
                    {searchQuery
                      ? 'No organizations match your search. Try another keyword.'
                      : 'Create the first organization to group users under.'}
                  </p>
                </div>
              ) : (
                <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                  {filteredOrgs.map((org) => {
                    ...
                    return (
                      <div
                        key={org.id}
                        className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
                      >
                        ...
                        <div className="p-6 pl-7">
                          <div className="mb-4 flex items-start justify-between">
                            ...
                            <button
                              ...
                              className="rounded-lg p-2 text-slate-300 opacity-0 transition-all group-hover:opacity-100 hover:bg-rose-50 hover:text-rose-600"
      ...
      <Dialog open={addOrgOpen} onOpenChange={setAddOrgOpen}>
        <DialogContent className="rounded-2xl sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new organization. Users can be assigned to this organization during registration.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setAddOrgOpen(false); setNewOrgName(''); setNewOrgDesc(''); }} className="rounded-xl">
              Cancel
            </Button>
      ...
      <Dialog open={addUserOpen} onOpenChange={setAddUserOpen}>
        <DialogContent className="rounded-2xl sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a user account and optionally assign them to an organization.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10"
              >
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setAddUserOpen(false); setNewUser({ firstName: '', middleName: '', lastName: '', email: '', password: '', org_id: '' }); }} className="rounded-xl">
              Cancel
            </Button>
      ...
      <AlertDialog open={deleteOrgDialog} onOpenChange={setDeleteOrgDialog}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the organization. Users assigned to it will become unassigned. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-xl">Cancel</AlertDialogCancel>
            <AlertDialogAction className="rounded-xl bg-rose-600 text-white hover:bg-rose-700" onClick={handleDeleteOrg}>
              Yes, delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
