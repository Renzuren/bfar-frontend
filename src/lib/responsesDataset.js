...
// Same normalisation questionMatches applies to both sides of a comparison.
const matchKey = (value) => String(value || '').trim().toLowerCase().replace(/[\s_-]+/g, '');

// A question's lookup keys, exactly the candidates questionMatches compares against.
const candidateKeys = (question) => [question.id, question.code, question.title]
  .filter(Boolean)
  .map((item) => String(item).trim().toLowerCase().replace(/[\s_-]+/g, ''));

// Lookup index over a list of (keys, payload) items, in order: every key and every
// prefix of it that ends right before a ':' maps to the FIRST item carrying it --
// the same match questionMatches finds (equal, or startsWith(`${candidate}:`)),
// without re-scanning and re-normalising every answer for every question.
const buildMatchIndex = (items) => {
  const index = new Map();
  items.forEach(({ keys }, position) => {
    keys.forEach((raw) => {
      const key = matchKey(raw);
      if (!index.has(key)) index.set(key, position);
      for (let i = key.indexOf(':'); i !== -1; i = key.indexOf(':', i + 1)) {
        const prefix = key.slice(0, i);
        if (!index.has(prefix)) index.set(prefix, position);
      }
    });
  });
  return index;
...
// First item (lowest position) matching any of the question's candidate keys.
const firstMatch = (indexed, question) => {
  let best = -1;
  candidateKeys(question).forEach((candidate) => {
    const position = indexed.index.get(candidate);
    if (position !== undefined && (best === -1 || position < best)) best = position;
  });
  return best === -1 ? undefined : indexed.items[best];
};

// Indexes are built once per answer source / response object and reused for every
// question (responses aren't mutated after loading). This turned building the
// 600-respondent Final Tilanggit export from ~13 s into a fraction of a second.
const sourceIndexCache = new WeakMap();
const sourceIndex = (source) => {
  if (!sourceIndexCache.has(source)) {
    const pairs = Array.isArray(source) ? source.map((value, index) => [index, value]) : Object.entries(source);
    const items = pairs.map(([key, value]) => ({
      value,
      keys: typeof value === 'object' && value !== null
        ? [value.question_id, value.qid, value.question_code, value.question_title]
        : [key],
    }));
    sourceIndexCache.set(source, { items, index: buildMatchIndex(items) });
  }
  return sourceIndexCache.get(source);
};
const directKeyCache = new WeakMap();
const directKeyIndex = (response) => {
  if (!directKeyCache.has(response)) {
    const items = Object.keys(response).map((key) => ({ key, keys: [key] }));
    directKeyCache.set(response, { items, index: buildMatchIndex(items) });
  }
  return directKeyCache.get(response);
};
...
export const answerFromAnyShape = (response, question, sections) => {
  ...
  for (const source of sources) {
    if (!source) continue;
    // Plain-text sources can't be cached by reference; scan them as before.
    const hit = typeof source === 'object'
      ? firstMatch(sourceIndex(source), question)
      : (() => {
        const found = Object.entries(source).find(([key]) => questionMatches(key, question));
  ...
  const directKey = firstMatch(directKeyIndex(response), question)?.key;
